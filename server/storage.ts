...
class DatabaseStorage implements IStorage {
  ...
  async getFinancialStatistics() {
    // Get total balance available for withdrawal across all users
    const totalBalance = await db.query.users.findMany({
      columns: {
        balance: true
      }
    });
    
    const totalAvailableBalance = totalBalance.reduce((sum, user) => {
      return sum + parseFloat(user.balance.toString() || '0');
    }, 0);

    // Get total amount already paid out from actual withdrawal payments
    ...
    const totalPaidOut = paidWithdrawals.reduce((sum, withdrawal) => {
      return sum + parseFloat(withdrawal.amount.toString() || '0');
    }, 0);

    // Get commission statistics from referrals
    ...
    const commissionStats = referrals.reduce((acc, referral) => {
      const indicatorCommission = parseFloat(referral.commissionIndicator || '0');
      const promoterCommission = parseFloat(referral.commissionPromoter || '0');
      
      acc.totalGenerated += indicatorCommission + promoterCommission;
      
      if (referral.status === 'converted' || referral.status === 'validated') {
        acc.totalEarned += indicatorCommission + promoterCommission;
      }
    ...
    const withdrawalStats = withdrawals.reduce((acc, withdrawal) => {
    ...
    return {
      ...
      totalPaidOut,
      totalCommissionsGenerated: commissionStats.totalGenerated,
